...
static inline float clamp(float v, float a, float b) { return std::min(std::max(v, a), b); } \
...
uint32_t Color::R11G11B10F(bool RoundToEven) const
{
    ...
    R.f = clamp(m_value.f[0], 0.0f, kMaxVal) * kF32toF16;
    G.f = clamp(m_value.f[1], 0.0f, kMaxVal) * kF32toF16;
    B.f = clamp(m_value.f[2], 0.0f, kMaxVal) * kF32toF16;
...
uint32_t Color::R9G9B9E5() const
{
    ...
    float r = clamp(m_value.f[0], 0.0f, kMaxVal);
    float g = clamp(m_value.f[1], 0.0f, kMaxVal);
    float b = clamp(m_value.f[2], 0.0f, kMaxVal);
